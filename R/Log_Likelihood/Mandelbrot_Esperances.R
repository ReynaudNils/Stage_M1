...
desc <- dplyr::desc
library(dplyr)
Data <- read.csv("Esperances.csv")
Data
Data$Rank<-rank( desc (Data$Esperances))
Data
fr <- unlist(Data['Esperances'])
p <- fr/sum(fr)
typeof(p)
len <- length(p)
typeof(fr)
...
opt <- optim(par = c(2, 4), opt.f, ower = c(0,0), method = "L-BFGS-B")
...
opt$par
coef(fit)
plot(1:len,p, xlab="Species", ylab="Mean Frequencies")
...
title(main = "Fitting Zipf-Mandelbrot for Mean Values of Generas")
legend(x = "topright", legend = c("Max Vraisemblance"), col = c("blue"), lty = 1, cex = 0.8)
